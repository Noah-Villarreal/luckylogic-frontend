...
const POWERBALL_HISTORY_KEY = 'powerballHistory';
...
export default function PowerballScreen() {
  ...
  const generatePick = (isDaily = false) => {
    ...
    try {
      const stored = localStorage.getItem(POWERBALL_HISTORY_KEY);
      const parsed = stored ? JSON.parse(stored) : [];
      const updated = [fullPick, ...parsed].slice(0, 100);
      localStorage.setItem(POWERBALL_HISTORY_KEY, JSON.stringify(updated));
    } catch (e) {
  ...
  return (
    <>
      <div className="App">
        ...
        {tab === 'history' && <HistoryList title="Powerball History" picks={getFilteredHistory()} renderPickRow={renderPickRow} />}
        {tab === 'personal' && (
          <PersonalInput
            showInput={showInput}
